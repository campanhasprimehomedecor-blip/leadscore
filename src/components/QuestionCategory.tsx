        ...
        {category.questions.map((q, idx) => {
          ...
          return (
            <div key={q.id} className="space-y-2 pb-4 border-b border-slate-100 last:border-b-0 last:pb-0">
              <label htmlFor={`select-${q.id}`} className="block text-sm font-medium text-slate-800 leading-snug">
                {idx + 1}. {q.text}
              </label>
              <div className="relative">
                <select
                  ...
                  className={`w-full py-2.5 px-3 pr-8 rounded-lg border text-sm font-medium bg-white focus:outline-hidden focus:ring-2 transition-all appearance-none cursor-pointer ${
                    selectedValue === ''
                      ? 'border-slate-300 text-slate-500 focus:ring-amber-500'
                      : isPenalty && selectedValue === 'sim'
                      ? 'border-red-300 bg-red-50/50 text-red-900 focus:ring-red-500'
